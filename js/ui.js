...
export function showResults(result) {
  ...
  result.values.forEach(value => { const card = document.querySelector(`.product[data-index="${value.index}"]`); const output = $('.unit-result',card); output.textContent = `${money(value.rate)} / ${value.unit}`; output.hidden = false; card.classList.toggle('winner', value.cheapest); const old = $('.badge', card); if(old) old.remove(); if(value.cheapest){const badge=document.createElement('span');badge.className='badge';badge.textContent='Best deal';$('.product-top',card).append(badge);} });
  const winners=result.values.filter(v=>v.cheapest); const base=unitInfo[result.values[0].unit].base; const winner=winners[0]; $('#result-summary').textContent = winners.length === 1 ? `${winner.name} is the best deal at ${money(winner.normalized)} per ${base}.${winner.saving ? ` That’s ${money(winner.saving)} less per ${base} than the next best option.` : ''}` : `It’s a tie — these options have the same price per ${base}.`; $('#results').hidden=false; return true;
}
export function clearComparison() { document.querySelectorAll('.unit-result').forEach(node=>{node.textContent='';node.hidden=true}); document.querySelectorAll('.badge').forEach(node=>node.remove()); document.querySelectorAll('.product').forEach(card=>card.classList.remove('winner')); $('#results').hidden=true; }
