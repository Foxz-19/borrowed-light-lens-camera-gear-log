...
describe("CabinetStorage", () => {
  ...
  it("reports corrupt data without silently reseeding", () => {
    ...
    memory.setItem("test", JSON.stringify([item, { bad: true }, { ...item }]));
    const result = new CabinetStorage(memory, "test").load();
    expect(result.items).toEqual([item]);
    expect(result.warning).toContain("Recovered 1 valid unique");
    memory.setItem("test", "bad");
    expect(new CabinetStorage(memory, "test").load().warning).toContain("data was invalid");
  });
