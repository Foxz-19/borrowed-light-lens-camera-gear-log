  ...
  <main class="workspace">
    <aside class="ledger" aria-labelledby="form-title">
      ...
      <form id="gear-form" novalidate>
        ...
        <div class="field-pair"><label>Category<select name="category" autocomplete="off" aria-describedby="category-error">${options(CATEGORIES, draft.category)}</select>${fieldError("category", errors)}</label><label>Condition<select name="condition" autocomplete="off" aria-describedby="condition-error">${options(CONDITIONS, draft.condition)}</select>${fieldError("condition", errors)}</label></div>
        ...
        <label class="borrower-field ${draft.isLent ? "is-visible" : ""}">Borrowed by<input name="borrower" maxlength="80" autocomplete="off" value="${escapeHtml(draft.borrower)}" aria-describedby="borrower-error" ${draft.isLent ? "required" : ""} />${fieldError("borrower", errors)}</label>
        <label>Personal note<textarea name="note" autocomplete="off" maxlength="240" rows="3" aria-describedby="note-help note-error">${escapeHtml(draft.note)}</textarea><span class="field-help" id="note-help">A memory, serial detail, or reason it matters · <span data-note-count>${draft.note.length}</span>/240</span>${fieldError("note", errors)}</label>
        <label>Date added<input type="date" name="dateAdded" autocomplete="off" value="${draft.dateAdded}" aria-describedby="dateAdded-error" required />${fieldError("dateAdded", errors)}</label>
    ...
    <section class="catalog" id="catalog" aria-labelledby="catalog-title">
      ...
      <div class="filters" aria-label="Catalog filters"><label><span>Find</span><input type="search" name="query-filter" autocomplete="off" value="${escapeHtml(filters.query)}" placeholder="Search name or note…" /></label><label><span>Category</span><select name="category-filter" autocomplete="off"><option value="all">All categories</option>${options(CATEGORIES, filters.category)}</select></label><label><span>Loan status</span><select name="loan-filter" autocomplete="off"><option value="all" ${filters.loan === "all" ? "selected" : ""}>All gear</option><option value="available" ${filters.loan === "available" ? "selected" : ""}>In my kit</option><option value="lent" ${filters.loan === "lent" ? "selected" : ""}>Lent out</option></select></label><button class="clear-filter" type="button" data-action="clear-filters">Clear</button></div>
